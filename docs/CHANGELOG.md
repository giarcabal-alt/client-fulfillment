...
### Fixed
- **Sidebar scrolled away with the page instead of staying pinned to the viewport — this was also the root cause of the earlier "sidebar background stops partway down" bug, confirmed and fixed together, not separately.** The `<aside>` in `(shell)/layout.tsx` had no explicit height, so the flex row's default `align-items: stretch` stretched it to match `<main>`'s full (potentially very tall, scrollable) content height instead of the viewport — meaning its painted background could run short of the viewport on a page shorter than one screen, and on any page it scrolled out of view with the rest of the document since it was still a normal in-flow element. Fixed with `sticky top-0 h-dvh` (not `h-screen`, so it also tracks mobile browser chrome resizing) — pins it to the viewport on scroll and gives it a fixed height stretch can no longer override. Also split the `<aside>`'s content into three regions (logo, a `min-h-0 flex-1 overflow-y-auto` nav section, greeting/sign-out) so that if the nav list — now 9 items for an admin (Board/Job Openings/Talent Bench/Rejected/Metrics/Clients/Settings/Admin/How to Use) — ever outgrows a short viewport, only the nav list scrolls internally; sign-out can never become unreachable. Confirmed live it doesn't need to today: at 1440x768 (a common laptop height) the full 9-item admin nav renders in 648px with zero overflow. Applied the identical fix to `MobileNav`'s slide-out `Sheet` (a separate component, per its own existing comment) — confirmed live via Playwright MCP that at a deliberately short 375x480 the nav list *does* correctly overflow and scroll internally while sign-out stays pinned and visible, proving the fallback path actually works, not just that it's present. Verified via Playwright MCP at 1440x900, 1440x768, and 375px (both a comfortable 700px and a stress-test 480px height): the sidebar's bounding rect stays fixed at the viewport's top/height while `window.scrollY` moves independently, on every page tested including the newly long `/help` page.
