...
## [Unreleased]
### Fixed (2026-09-25 full regression pass)
- **Clients table never got a delete RLS policy, so "unblocked" delete (a client with no linked roles) silently no-ops instead of deleting.** `20260923100000_clients_and_job_openings_expansion.sql` deliberately deferred adding one ("same reasoning as roles originally — add a delete policy only once something actually needs it"), but `deleteClientRecord` (`clients-actions.ts`) shipped since and the Clients page has a working Delete button with its own blocked/unblocked-path logic already correctly coded. Without a matching RLS policy, Postgres's default-deny-with-RLS-enabled behavior means `.delete()` isn't rejected — it just deletes 0 rows and returns no error — so the app reports success, closes the confirm dialog, and the row silently stays. Caught during this regression pass: created a client with no linked roles, deleted it through the UI, dialog closed cleanly with no error, but the row was still there after a hard reload. Root cause confirmed by reading the original migration's own comment. Fixed with a new migration, `20260925120000_add_clients_delete_policy.sql` (`create policy "clients: delete within org" on clients for delete using (org_id = public.current_org_id())`), matching the exact pattern `roles: delete within org` already established. **Migration file is written but not yet applied to the live database** — schema changes to shared infrastructure weren't pushed unilaterally; needs `supabase db push` (or equivalent) before the Delete button on Clients actually works. The blocked path (client with linked roles) already worked correctly before this fix, since that case is rejected in application code before ever reaching `.delete()`.

### Notes from this regression pass (not code fixes)
- **Admin → Invite a teammate fails for every recipient, including plain `@gmail.com` addresses** — `AuthRetryableFetchError: Error sending invite email`, `status: 500`, from `supabase.auth.admin.inviteUserByEmail`. Not domain-specific (retested with a second, ordinary address, same failure), so this is a Supabase-project-level email-sending problem (likely rate-limited or SMTP not configured), not an app bug — no code change made. This also blocks live verification of the Reset Password email path, which goes through the same underlying send. Needs to be checked in the Supabase project's Auth → Email settings.
- **Role delete (Job Openings) has no confirmation step** — one click permanently deletes a role, unlike Clients' delete which has both a confirm dialog and a linked-record block. Flagging since it's inconsistent with the rest of the app's destructive-action pattern, not because it was asked to change.
- **No manual "add a skill" affordance anywhere in the app** (roles or candidates) — skills can only be added via Parse JD/resume-autofill, then edited or removed per-chip. Confirmed this is consistent, existing behavior across both `role-skills-editor.tsx` and the candidate equivalent, not a regression — flagging since the regression-test brief asked to verify "skills added manually" and there's currently no such control to test.
- Full pass otherwise confirmed clean: Board (search/filter/stage-change/cadence-reset), Job Openings (create with every field via Parse JD + client link, status→`role_history` logging, full titles with no truncation), Candidates (manual entry, PDF **and** DOCX resume autofill with edits respected, all detail-page properties/tabs including Skills & Fit match/needs-review, Outreach script + AI-generated message, Scorecards, History, Reject-requires-reason-and-removes-from-Board), Talent Bench/Rejected (full roster regardless of stage, filters, no truncation), Metrics (plausible numbers, correct decline-reason grouping), Clients (edit + timezone-driven live clock, delete correctly blocked with a clear message for a client with linked roles), Admin (role change both directions works, self-demotion blocked with a clear message, last-admin-demotion logic present and correct by code review though not independently UI-triggerable in a single-admin-actor session, non-admin `/admin` access is a real `notFound()` not just a hidden link), Settings (both fields save and propagate — display name updates the sidebar greeting live), How to Use (loads, sidebar link works), `npx vitest run` (44/44 passing), keyboard focus rings (verified via real Tab key presses, not programmatic `.focus()`, on a board card — the ring correctly renders on the parent `Card` via `has-[:focus-visible]` so it isn't clipped by the card's own overflow — the New Candidate form, a candidate-detail tab, and an Admin button), and 375px mobile (Board, a candidate detail page, and Clients — the Clients table's Edit/Delete columns are reachable via the table's own `overflow-x-auto`, matching the documented "tables may exceed the page width inside their own scroll container" pattern).

### Added
- **New `/help` page — "How to Use" — a real in-app route, not an external doc, since this is an internal tool and whoever needs it is already logged in.** App-level (sidebar item below the divider, next to Settings and Admin — same group in both the desktop sidebar and the mobile Sheet, since `MobileNav` renders the same `SidebarNav`), not nested under `talent-acquisition/`, matching how Settings isn't module-specific either. `Section` gained an optional `id` prop (plus `scroll-mt-4`) so its sections can be anchor-linked from the page's table of contents — a small, backward-compatible addition, not a one-off for this page. Full width per this task's explicit instruction rather than the 1120px reading-width cap DESIGN_SYSTEM.md §4 otherwise reserves for long-form text — a sticky left-column table of contents is what makes the extra width earn its keep on a page that's mostly prose. Covers the Board (seven stages including Talent Pool, the overdue/soon/on-track/parked status badges and the cadence they're driven by, search), Job Openings (creating a role, linking/creating a client inline, Parse JD vs. manual skill chips, every field), adding a candidate (manual vs. resume-upload autofill — what pre-fills and what needs review), the candidate detail page (properties, Skills & Fit, Outreach's script + AI-generated draft, Scorecards, History, Reject), Talent Bench (roster view vs. the Board's pipeline view), Rejected (same roster, pre-filtered, kept not deleted), Metrics (time-in-stage, decline reasons), Clients (the table, the Edit dialog, the timezone dropdown driving the live clock), and Admin (linked to rather than re-explained, per this task's own instruction not to duplicate that page's content). Verified live via Playwright MCP at 1440x900 and 375px: the sidebar/mobile-Sheet link works and shows the active state, the two-column TOC-plus-content layout collapses to one column on mobile with no horizontal overflow, and anchor scrolling lands correctly on each section.
...
### Fixed
- **Sidebar scrolled away with the page instead of staying pinned to the viewport — this was also the root cause of the earlier "sidebar background stops partway down" bug, confirmed and fixed together, not separately. Took two passes; the second pass is what actually fixed it — see the correction below.** The `<aside>` in `(shell)/layout.tsx` had no explicit height, so the flex row's default `align-items: stretch` stretched it to match `<main>`'s full (potentially very tall, scrollable) content height instead of the viewport — meaning its painted background could run short of the viewport on a page shorter than one screen, and on any page it scrolled out of view with the rest of the document since it was still a normal in-flow element. Fixed with `sticky top-0 h-dvh` (not `h-screen`, so it also tracks mobile browser chrome resizing) — pins it to the viewport on scroll and gives it a fixed height stretch can no longer override. Split the `<aside>`'s content into three regions (logo, a `min-h-0 flex-1 overflow-y-auto` nav section, greeting/sign-out) so that if the nav list — now 9 items for an admin (Board/Job Openings/Talent Bench/Rejected/Metrics/Clients/Settings/Admin/How to Use), 11 rows counting the two disabled "Coming soon" placeholders — ever outgrows a short viewport, only the nav list scrolls internally; sign-out can never become unreachable. Applied the identical fix to `MobileNav`'s slide-out `Sheet`, a separate component.
  - **Correction — first pass looked right but wasn't actually confirmed visually, and the real bug was different from what it looked like on paper.** The first pass was verified only via `getBoundingClientRect()`/computed-style checks (Playwright's screenshot tool was failing that session) — those checks were numerically correct (Sign Out's rect genuinely sits inside the viewport at every tested size), so the *flex distribution itself was never actually broken*. What screenshots later revealed: at the sidebar's default `p-3` padding, Sign Out lands close enough to the exact bottom-left corner that Next.js's dev-mode route indicator (which also defaults to `bottom-left`) rendered directly on top of it — visually and functionally unreachable in the dev environment (not a production issue; the indicator doesn't exist in a production build, but a real problem for anyone developing this app locally). Fixed by moving the indicator out of the way (`devIndicators: { position: "bottom-right" }` in `next.config.ts`) rather than padding the sidebar to dodge a dev-only overlay. Also hardened the three-region structure to match the spec literally — the logo region gained an explicit `shrink-0` wrapper (it previously relied on default flex sizing happening to work out, not on an explicit guarantee) in both `(shell)/layout.tsx` and `MobileNav`. **The lesson for future sessions**: "make the sidebar sticky" was necessary but not sufficient — the three-region `shrink-0` logo / `flex-1 overflow-y-auto` nav / `shrink-0` footer structure is what actually guarantees Sign Out stays reachable, and *visual* verification (real screenshots, not just computed rects) is what caught the dev-indicator overlap that numeric checks alone missed. Re-verified live via Playwright MCP with actual screenshots (not just computed values) at 1440x900 (scrolled 1500px into the now-long `/help` page — sidebar and Sign Out stay put), 1440x768 (a common laptop height — Sign Out fully visible, dev indicator now bottom-right, no overlap), and 375px mobile (a comfortable 800px height, and a deliberate 480px stress height where the nav list visibly scrolls internally while the logo and Sign Out stay fixed and visible throughout).
...
### Changed
- **Layout pass: dropped the 1120px page cap, rebuilt four pages (role detail, Job Openings, candidate detail, Clients) to use the full width beside the sidebar — closes out the heavy-scrolling/wasted-space problem the earlier density passes didn't touch.** `DESIGN_SYSTEM.md` §4/`DESIGN.md`'s Layout section amended first (dated decision logged in `PROJECT_STATE.md` §8): the 1120px max content width, inherited from the brand guide's marketing-site context, now applies only to long-form reading text (a job description block) — app screens use the full available width with the existing `p-4`/`sm:p-6` gutter. Every page committed and verified separately via Playwright MCP at 1440x900, 1280x800, and 375px.
  - **Role detail (`/talent-acquisition/roles/[id]`)**: header row (editable title/status/priority + client name link with the live PH/client clock, removing the separate duplicate Client card), left column keeps every property via the compact `PropertyRow` pattern, right column is now tabbed (Skills/Job Description/Candidates/History) so the page fits mostly in one 1440x900 viewport instead of the old single narrow scrolling column. Job Description gets its own small client component with the 1120px reading-width cap.
  - **Job Openings (`/talent-acquisition/roles`)**: full-width table — title (an auto-growing `<textarea rows={1}>`, not an `<input>`, since an input can never wrap; long titles now wrap and grow the row instead of showing `"Senior Full Stack Devel…"`), client, status, classification, priority, target fill date, candidate count, actions. Job description stays a click-to-expand preview row rather than a dedicated column.
  - **Candidate detail (`/talent-acquisition/candidates/[id]`)**: `PropertyRow` gained an optional `labelClassName` override (default unchanged) — this page uses a 130px label column (was ~152px) so values stop truncating (`"AI Solutions Engi…"`, `"Quezon City, Met…"` are gone). Select-based rows override the shared trigger's forced single-line ellipsis so long values wrap and the trigger grows; free-text fields most likely to run long (last role, last company, expected compensation) become auto-growing textareas, same fix as the Job Openings title. Page goes full width with a fixed 420px left column so the tabs get much more room; "Assigned to" confirmed visible without scrolling at 1440x900.
  - **Clients**: the standalone `/talent-acquisition/clients/[id]` detail page is gone (old links now redirect to the list) — every field renders inline on the list as a full-width table (company, industry, website, location, timezone with the live clock, point of contact, linked roles each linking to its role, notes), edited via a single "Edit" dialog covering every field at once instead of the old page's per-field save-on-blur. Timezone is now a grouped IANA-zone dropdown (`client-timezones.ts`, by region with readable labels) instead of free text, so daylight saving is handled automatically — an existing stored value outside the fixed list falls back to "Not set" in the dropdown without breaking the live clock, which still reads the raw stored string. Added admin-only client deletion (`requireAdminUser()`) with a confirmation dialog; since `roles.client_id` is `ON DELETE SET NULL` (would silently unlink roles rather than error), `deleteClientRecord` pre-checks for linked roles and blocks the delete with a clear count-based message — verified live against a client with a real linked role.
